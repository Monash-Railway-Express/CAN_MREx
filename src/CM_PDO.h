...
struct PdoMap {
    uint8_t count;
    PdoMapEntry
        e[8];  // up to 8 entries (<= 64 bytes total for CAN FD, but we’ll cap to 8 bytes classic)
};
...
// Call in loop
void _ProcessRPDO(const twai_message_t& rx, uint8_t nodeID);  // now includes nodeID
void _ServiceTPDOs(uint8_t nodeID);                           // handles periodic/event-driven sends

// Helpers
bool packTPDO(uint8_t nodeID, uint8_t pdoNum, uint8_t* outBytes,
              uint8_t* outLen);  // now includes nodeID
bool unpackRPDO(uint8_t nodeID, uint8_t pdoNum, const uint8_t* data,
                uint8_t len);  // now includes nodeID
