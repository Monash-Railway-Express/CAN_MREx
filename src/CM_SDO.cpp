...
void _StoreSDOResponse(const twai_message_t& msg) {
    sdoResponse = msg;
    sdoResponseReady = true;
}

// Handle SDO transmits
void _HandleSDO(const twai_message_t& rxMsg, uint8_t nodeID) {
...
void executeSDOWrite(uint8_t nodeID, uint8_t targetNodeID, uint16_t index, uint8_t subindex,
...
void prepareSDOTransmit(uint8_t cmd, uint16_t index, uint8_t subindex, const void* value,
                        size_t size, uint8_t* outBuf) {
    outBuf[0] = cmd;
    outBuf[1] = index & 0xFF;
...
void waitSDOResponse(uint32_t* outValue, uint8_t targetNodeID, uint8_t nodeID) {
            ...
            if (cmd == 0x4F || cmd == 0x4B || cmd == 0x43) {
                ...
                switch (cmd) {
                    case 0x4F:
                        ...
                        break;
                    case 0x4B:
                        value = response.data[4] | (response.data[5] << 8);
                        break;
                    case 0x43:
                        value = response.data[4] | (response.data[5] << 8) |
    ...
    uint32_t errorCode = (0x00000008) | ((uint32_t)targetNodeID << 8) | ((uint32_t)nodeID << 16);
    sendEMCY(0x00, nodeID, errorCode);  // SDO response not received
}