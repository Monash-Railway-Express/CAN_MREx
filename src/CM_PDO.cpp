...
void initDefaultPDOs(uint8_t nodeID) {
    for (int i = 0; i < 4; i++) {
        setComm(tpdoComm[i], 0x80000000u | (0x180 + (i * 0x100) + nodeID), 255, 0,
                0);  // disabled by bit31
...
static bool readMapped(const PdoMapEntry& me, uint8_t* out, uint8_t& offsetBytes) {
    ...
    if (!od || (me.len_bits % 8) != 0)
        return false;
    uint8_t n = me.len_bits / 8;
    if (offsetBytes + n > 8)
        return false;  // classic CAN
...
void _ProcessRPDO(const twai_message_t& rx, uint8_t nodeID) {
...
void markTpdoDirty(uint8_t pdoNum) {
    if (pdoNum < 4)
        tpdoDirty[pdoNum] = true;
}

// Services all TPDOs: checks timers, dirty flags, inhibits, and transmits if due
void _ServiceTPDOs(uint8_t nodeID) {
    uint32_t now = millis();

    for (uint8_t i = 0; i < 4; i++) {
        if (!tpdoComm[i].enabled)
            continue;
        ...
        if (tpdoComm[i].event_timer > 0) {
            if (now - tpdoState[i].last_tx_ms >= tpdoComm[i].event_timer)
                due = true;
        }
        ...
        if (tpdoComm[i].inhibit_time > 0) {
            if (now - tpdoState[i].last_tx_ms < tpdoComm[i].inhibit_time)
                continue;
        }

        uint8_t payload[8] = {0};
        uint8_t len = 0;
        if (!packTPDO(nodeID, i, payload, &len))
            continue;
...
void configureTPDO(uint8_t pdoNum, uint32_t cobID, uint8_t transType, uint16_t inhibitMs,
                   uint16_t eventMs) {
    if (pdoNum < 4) {
