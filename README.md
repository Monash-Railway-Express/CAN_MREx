# CAN_MREX
CAN MREX is a CAN wrapper that is intended to adhere to some of the CAN open standards however is designed specifically for Monash Railway Express's locomotive.


## Helpful resources:
Simple intro - Good for first read
Simple CAN Open intro and tutorial - Good for learning what CAN open is
Cia full documentation - 301 
Esp32 twai drivers
CAN bus
...
Controls node states (e.g. Operational, Preoperational, Stopped). Sent by the Node manager to manage other nodes.
SYNC - Not implemented
Synchronizes actions across nodes. Often triggers PDO transmission. 
EMCY (Emergency)
Broadcasts fault conditions instantly. Used for fast error reporting.
TPDO (Transmit)
Sends real-time process data (e.g. sensor values, target speed) from node to bus. Use this for data we want to monitor constantly.
RPDO (Receive)
Receives process data (e.g. actuator commands) from bus to node. It will look out for specified TPDOs and take that from the bus into the node.
SDO
Sends structured data (e.g. config parameters) from node to another with a confirmation (Client server model). Can also request data from another node.
Heartbeat / Node Guarding
Monitors node presence and status. Detects if a node goes offline or fails.
...
This link provides a deeper insight
...
https://docs.google.com/spreadsheets/d/1OaXG5B06xnvpNkGQIkrtbM_n-pCCqvnd99yezD7YYoQ/edit?gid=0#gid=0 
CAN parameters
...
Also please make sure you are using uint8_t, uint16_t and unit32_t for variables cause the can bus only accepts unsigned ints. Talk to me if you’re worried about this.
Object Dictionary
...
Size
Data
2 byte value e.g 0x1000
1 byte value  e.g 0x01
0 = Read only,
1 = Write only, 
2 = Read Write
Size of data being stored (should usually be less than 8 bytes)
...
Initialise variables you want in the object dictionary. They must be of byte size 1, 2 or 4. (unint8_t, unint16_t, unint32_t). This allows you to access the variables normally in your program.  Ensure they are unsigned integers!!! Talk to me if you want to use floats or negative numbers we can work something out. 

Register the OD entries using the above table to help identify what variables you need to pass to the function.


Object Dictionary lookup 
6040h: Controlword: Used to control the operational state of the drive.
6041h: Statusword: Provides information about the current status of the drive.
6060h: Modes of operation: Defines the current operating mode of the motor drive.
6064h: Position actual value: Reads the actual position of the motor.
607Ah: Target position: Used to set the target position of the motor.
60FFh: Target velocity: Used to set the target velocity of the motor.
Add here link to excel
https://docs.myostat.ca/cm1-e/user-guide/object-dictionary#ObjectDictionary-0x606C:Speedactualvalue 
Transmit types
SDOs
...
Purpose
0
Command specifier (e.g., read/write)
1–2
Index (object dictionary)
3
Subindex
4–7
Data (up to 4 bytes for expedited transfer)
...
Data Size
0x2F
Write 1 byte (3 unused)
1 byte
0x2B
Write 2 bytes (2 unused)
2 bytes
0x23
Write 4 bytes (0 unused)
4 bytes
0x40
Read request
0 bytes
...
Size of value
Own node id
The node you want to write to
The index in the targeted nodes OD you want to write to 
The subindex
The value you want to write
The size of that value (1, 2 or 4 bytes)
...
The index in the targeted nodes OD you want to write to 
The subindex
The value you will receive into Warning! It will always be a uint32 if you want it to be of a different size you will have to typecast
...
Inihibit
eventMs
Selects which PDO channel to configure (0–3)
0 for TPDO1 / RPDO1
CAN identifier used for the PDO. 

Bit 31 disables the PDO if set.
Transmission type: defines when the PDO is sent

255 - asynchronous (what we’ll be using)

1 - (synchronous every cycle) (not implemented)
Minimum time (in ms) between transmissions to prevent flooding
Optional timer for periodic transmission

0 (disabled), 100 (send every 100 ms)
...
Bit 31 disables the PDO if set.
Transmission type: defines when the PDO is expected 
...
0 for TPDO1 / RPDO1
Pointer to array of PdoMapEntry structs defining mapped variables

{ {0x0001, 0x00, 8}, {0x1000, 0x00, 8} }
Number of mapped entries in the array
...
Reset node
0x82
Reset communication (not implemented)
...
Purpose
0
Node state (e.g., operational, pre-operational)
...
Purpose
0
Error priority (0 for major,  1 for minor) 
1
Node location
2-5
Error code (4 bytes) (little endian) I might change this for ease of reading
...
https://docs.google.com/spreadsheets/d/1OaXG5B06xnvpNkGQIkrtbM_n-pCCqvnd99yezD7YYoQ/edit?gid=1817306757#gid=1817306757 
