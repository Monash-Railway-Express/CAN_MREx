...
void setup() {
    ...
    registerODEntry(0x2000, 0x01, 2, sizeof(speed),
                    &speed);  // Index, Subindex, Read Write access, Size, Data
    registerODEntry(0x2001, 0x00, 2, sizeof(brake),
                    &brake);  // Index, Subindex, Read Write access, Size, Data
...
void loop() {
    switch (currentTest) {
        ...
        case TEST_SDO: {
            ...
            executeSDOWrite(
                nodeID, 2, 0x0001, 0x00, sizeof(mode),
                &mode);  // Node ID, targeted node, index, subindex, value, size of value

            Serial.println("Reading SDO...");
            uint8_t confirmation = 0;
            // Confirm it's changed
            confirmation =
                executeSDORead(nodeID, 2, 0x0001, 0x00);  // Node ID, targeted node, index, subindex

            if (mode == confirmation)
                printPass("SDO read/write OK");
            else
                printFail("SDO read/write failed");
        ...
        case TEST_TPDO: {
            ...
            confirmSpeed32 =
                executeSDORead(nodeID, 2, 0x2000, 0x01);  // Node ID, targeted node, index, subindex
            confirmBrake32 =
                executeSDORead(nodeID, 2, 0x2001, 0x00);  // Node ID, targeted node, index, subindex
            uint16_t confirmSpeed16 = (uint16_t)confirmSpeed32;
            uint8_t confirmBrake8 = (uint8_t)confirmBrake32;

            if (confirmSpeed16 == speed && confirmBrake8 == brake)
                printPass("PDOs OK");
            else
                printFail("PDOs failed");
        ...
        case TEST_NMT: {
            ...
            node2_operating_mode =
                executeSDORead(nodeID, 2, 0x1000, 0x00);  // Node ID, targeted node, index, subindex
            if (node2_operating_mode == 0x80)
                printPass("PRE-OP accepted");
            else
                printFail("PRE-OP failed");
            ...
            node2_operating_mode =
                executeSDORead(nodeID, 2, 0x1000, 0x00);  // Node ID, targeted node, index, subindex
            if (node2_operating_mode == 0x01)
                printPass("Operational accepted");
            else
                printFail("Operational failed");
