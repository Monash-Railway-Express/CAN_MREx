...
void handleCAN(uint8_t nodeID, twai_message_t* pdoMsg) {
    ...
    checkHeartbeatTimeouts();  // Checks heartbeats to make sure they're not overdue (Heartbeat
                               // consumer only)
    ...
    if (canID == 0x000) {  // NMT commands (always processed)
        ...
        return;
               ...
               nodeOperatingMode == 0x01) {  // RPDOs (only in operational state)
        ...
        return;
               ...
               (nodeOperatingMode == 0x01 || nodeOperatingMode == 0x80)) {  // SDO transmits
